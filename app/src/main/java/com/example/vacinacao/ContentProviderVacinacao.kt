...
class ContentProviderVacinacao : ContentProvider() {
    private var bdVacinacaoOpenHelper: BdVacinacaoOpenHelper? = null
    ...
    override fun onCreate(): Boolean {
        bdVacinacaoOpenHelper = BdVacinacaoOpenHelper(context)
 ...
 * and Threads]({@docRoot}guide/topics/fundamentals/processes-and-threads.html#Threads).
    ...
    override fun query(
        ...
        val bd = bdVacinacaoOpenHelper!!.readableDatabase

        return when (getUriMatcher().match(uri)) {
            URI_FICHAS -> TabelaFicha(bd).query(
            ...
            URI_FICHA_ESPECIFICA -> TabelaFicha(bd).query(
                projection as Array<String>,
                "${BaseColumns._ID}=?",
            ...
            URI_PACIENTES -> TabelaPaciente(bd).query(
                projection as Array<String>,
                selection,
                selectionArgs as Array<String>?,
                null,
            ...
            URI_VACINAS -> TabelaVacina(bd).query(
                projection as Array<String>,
                selection,
                selectionArgs as Array<String>?,
                null,
 ...
 * and Threads]({@docRoot}guide/topics/fundamentals/processes-and-threads.html#Threads).
    ...
    override fun getType(uri: Uri): String? {
        return when (getUriMatcher().match(uri)) {
            URI_FICHAS -> "$MULTIPLOS_ITEMS/$FICHAS"
            URI_FICHA_ESPECIFICA -> "$UNICO_ITEM/$FICHAS"
 ...
 * and Threads](
    ...
    override fun insert(uri: Uri, values: ContentValues?): Uri? {
        val bd = bdVacinacaoOpenHelper!!.writableDatabase

        val id = when (getUriMatcher().match(uri)) {
            URI_FICHAS -> TabelaFicha(bd).insert(values!!)
            URI_PACIENTES -> TabelaPaciente(bd).insert(values!!)
            URI_VACINAS-> TabelaVacina(bd).insert(values!!)
            else -> -1L
        }
 ...
 * and Threads](
    ...
    override fun delete(uri: Uri, selection: String?, selectionArgs: Array<out String>?): Int {
        ...
        return when (getUriMatcher().match(uri)) {
            URI_FICHA_ESPECIFICA -> TabelaFicha(bd).delete(
                "${BaseColumns._ID}=?",
                arrayOf(uri.lastPathSegment!!)
            )
 ...
 * and Threads](
    ...
    override fun update(
        ...
        return when (getUriMatcher().match(uri)) {
            URI_FICHA_ESPECIFICA -> TabelaFicha(bd).update(
                values!!,
                "${BaseColumns._ID}=?",
                arrayOf(uri.lastPathSegment!!)
            )
    ...
    companion object {
        ...
        private const val URI_FICHAS = 100
        private const val URI_FICHA_ESPECIFICA = 101
        ...
        public val ENDERECO_FICHAS = Uri.withAppendedPath(ENDERECO_BASE, FICHAS)
        ...
        private fun getUriMatcher(): UriMatcher {
            ...
            uriMatcher.addURI(AUTHORITY, FICHAS, URI_FICHAS)
            uriMatcher.addURI(AUTHORITY, "$FICHAS/#", URI_FICHA_ESPECIFICA)
